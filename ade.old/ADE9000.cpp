...
ADE9000::ADE9000(uint8_t spiPort) : port(spiPort)
{
	_chipSelect_Pin = 25;
	m_flipCurr = false;
}
...
void ADE9000::SPI_Init(uint32_t SPI_speed, uint8_t chipSelect_Pin)
{
	...
	port.beginTransaction(SPISettings(SPI_speed, MSBFIRST, SPI_MODE0));		//Setup SPI parameters
...
/*
Description: Initializes the arduino SPI port using SPI.h library
Input: SPI speed, chip select pin, clock pin, mosi pin (out), miso pin (input)
Output:-
*/
void ADE9000::SPI_Init(uint32_t SPI_speed, uint8_t chipSelect_Pin, uint8_t clkPin, uint8_t misoPin, uint8_t mosiPin)
{
	port.begin(clkPin, misoPin, mosiPin);
	port.beginTransaction(SPISettings(SPI_speed, MSBFIRST, SPI_MODE0));		//Setup SPI parameters
	pinMode(chipSelect_Pin, OUTPUT);		//Set Chip select pin as output	
	digitalWrite(chipSelect_Pin, HIGH);		//Set Chip select pin high 
...
void ADE9000::SPI_Write_32(uint16_t Address, uint32_t Data)
{
	...
	temp_address = ((Address << 4) & 0xFFF0);	//shift address  to align with cmd packet
	port.transfer16(temp_address);
	port.transfer16(temp_highpacket);
	port.transfer16(temp_lowpacket);
...
uint32_t ADE9000::SPI_Read_32(uint16_t Address)
{
	...
	temp_address = (((Address << 4) & 0xFFF0) + 8);
	port.transfer16(temp_address);
	temp_highpacket = port.transfer16(0);
	temp_lowpacket = port.transfer16(0);
...
void ADE9000::SPI_Burst_Read_Resampled_Wfb(uint16_t Address, uint16_t Read_Element_Length, ResampledWfbData* ResampledData)
{
	...
	{
		ResampledData->IA_Resampled[i] = port.transfer16(0);
		ResampledData->VA_Resampled[i] = port.transfer16(0);
		ResampledData->IB_Resampled[i] = port.transfer16(0);
		ResampledData->VB_Resampled[i] = port.transfer16(0);
		ResampledData->IC_Resampled[i] = port.transfer16(0);
		ResampledData->VC_Resampled[i] = port.transfer16(0);
		ResampledData->IN_Resampled[i] = port.transfer16(0);
	}
	digitalWrite(_chipSelect_Pin, HIGH);
}
